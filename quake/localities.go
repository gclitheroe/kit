...
var localities = []Locality{
	...
	{Name: `Lower Hutt`, Longitude: 174.91, Latitude: -41.21, size: 1},
	{Name: `Upper Hutt`, Longitude: 175.07, Latitude: -41.12, size: 1},
	{Name: `Porirua`, Longitude: 174.84, Latitude: -41.13, size: 1},
...
func (q *Quake) Closest() LocalityQuake {
	distance := 20000.0
	var bearing float64
	...
	for _, l := range localities {
		// TODO ignored error
		d, b, _ := distanceBearing(l.Latitude, l.Longitude, q.Latitude, q.Longitude)
		if d < distance {
			distance = d
			locality = l
			bearing = b
	...
	var loc LocalityQuake
	loc.Locality = locality
	loc.Distance = distance
	loc.Bearing = bearing

	// TODO MMIdistance in locality.

	return loc
	//return MMIDistance(distance, q.Depth, q.MMI())
}
